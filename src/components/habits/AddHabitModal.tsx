...
export function AddHabitModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Thêm Thói Quen Mới" maxWidth="md">
      <form onSubmit={handleSubmit} className="space-y-4">
        <Input label="Tên thói quen *" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Ví dụ: Đọc sách 30 phút" required />
        ...
        <Input label="Mô tả (tuỳ chọn)" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Động lực của bạn..." />
        ...
        <div className="flex justify-end gap-2 pt-4 border-t border-stone-100 dark:border-stone-800">
          ...
          <Button type="submit" disabled={isSubmitting || !title.trim()} className="bg-[#1e3a2f] hover:bg-[#152a22] text-white gap-2">
            {isSubmitting ? <><Loader2 className="h-4 w-4 animate-spin" /><span>Đang lưu...</span></> : <><Check className="h-4 w-4" /><span>Lưu thói quen</span></>}
          </Button>
        </div>
      </form>
    </Modal>
