...
export function AddJournalModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Viết Nhật Ký" maxWidth="2xl">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Hôm nay bạn cảm thấy thế nào? Có điều gì đặc biệt xảy ra không?"
            required
            rows={8}
          />
        </div>
        ...
        <div className="flex justify-end gap-2 pt-4 border-t border-stone-100 dark:border-stone-800">
          ...
          <Button type="submit" disabled={isSubmitting || !content.trim()} className="bg-[#1e3a2f] hover:bg-[#152a22] text-white gap-2">
            {isSubmitting ? <><Loader2 className="h-4 w-4 animate-spin" /><span>Đang lưu...</span></> : <><Check className="h-4 w-4" /><span>Lưu nhật ký</span></>}
          </Button>
        </div>
      </form>
    </Modal>
